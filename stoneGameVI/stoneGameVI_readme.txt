...
2. For every turn, find the index of the largest sum of aliceValues[i] and bobValues[i]
3. Using the above index, add their points to their sum
4. Dynamic Programming step: change aliceValues[i] and bobValues[i] at this index to zero
5. Repeat 1-4 until the pile is empty
6. Return the winner/draw
This method is inefficient due to the massive search-log and nested comparisons of sums from the input arrays
...
3. Sort val2[] in descending order, while also making the same sort changes to the values in order[]
4. Iterate along order, adding aliceValues[i] and bobValues[i] along order
5. Return the winner/draw
This method failed because with large input arrays the sorting algorithm becomes bogged down
